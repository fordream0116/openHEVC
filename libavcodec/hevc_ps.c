...
static int decode_profile_tier_level(HEVCLocalContext *lc,  ProfileTierLevel *ptl)
{
    int i, j;
    GetBitContext *gb = &lc->gb;
...
static void parse_vps_extension (HEVCContext *s, HEVCVPS *vps)  {
...
#endif
    {
        ...
        vps->PTLExt[i] = av_malloc(sizeof(PTL));  // TO DO add free
        parse_ptl(s->HEVClc, vps->PTLExt[i], vps->vps_max_sub_layers);
    }
#endif
...
int ff_hevc_decode_nal_sps(HEVCContext *s)
{
    ...
    if (parse_ptl(lc, &sps->ptl, sps->max_sub_layers) < 0) {
        av_log(s->avctx, AV_LOG_ERROR, "error decoding profile tier level\n");
        ret = AVERROR_INVALIDDATA;
        goto err;
    }
